...
public:
  ...
  void AddPoint(std::vector<double> point, size_t ind) 
  {
    ...
    if ( (pointcloud_type_ == "XYZIR"  && point.size() != 5) ||
      ...
      throw std::runtime_error("Point has unsuported size = " + std::to_string(point.size()) + 
...
inline void FillPointCloud(PointCloud* pc, size_t points_count, std::string point_type, 
